...
class LargeFilterSizesVAE(BaseVAE):

    def __init__(self, model_parameters: dict):
        super().__init__(model_parameters)

        assert len(self.hidden_dimensions) == 5, "For the chosen VAE architecture, 5 hidden dimensions are needed"
    ...
    def decode(self, z: torch.Tensor) -> torch.Tensor:
        x = self.fc_decoder(z)
        x = x.view(-1, self.hidden_dimensions[4], 6, 6)
