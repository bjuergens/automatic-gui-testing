...
class SmallFilterSizesSmallBottleneckVAE3(BaseVAE):
    ...
    def __init__(self, model_parameters: dict):
        ...
        conv_7 = nn.Conv2d(in_channels=self.hidden_dimensions[5], out_channels=self.hidden_dimensions[6], kernel_size=3, stride=2, padding=1)
        ...
        # Bottleneck Size: 2x2
        self.fc_mu = nn.Linear(2 * 2 * self.hidden_dimensions[7], self.latent_size)
        self.fc_log_var = nn.Linear(2 * 2 * self.hidden_dimensions[7], self.latent_size)

        self.fc_decoder = nn.Linear(self.latent_size, 2 * 2 * self.hidden_dimensions[7])
        ...
        transposed_conv_0 = nn.ConvTranspose2d(in_channels=self.hidden_dimensions[6], out_channels=self.hidden_dimensions[5], kernel_size=3, stride=2, padding=1, output_padding=0)
    ...
    def decode(self, z: torch.Tensor) -> torch.Tensor:
        x = self.fc_decoder(z)
        x = x.view(-1, self.hidden_dimensions[7], 2, 2)
...
def main():
    ...
    model = SmallFilterSizesSmallBottleneckVAE3({
        "input_channels": 3,
        "latent_size": 32,
