...
class HalfInputSmallFilterSizesSmallBottleneckVAE(BaseVAE):
    ...
    def __init__(self, model_parameters: dict):
        ...
        # Bottleneck Size: 6x6
        self.fc_mu = nn.Linear(4 * 4 * self.hidden_dimensions[5], self.latent_size)
        self.fc_log_var = nn.Linear(4 * 4 * self.hidden_dimensions[5], self.latent_size)

        self.fc_decoder = nn.Linear(self.latent_size, 4 * 4 * self.hidden_dimensions[5])
    ...
    def decode(self, z: torch.Tensor) -> torch.Tensor:
        x = self.fc_decoder(z)
        x = x.view(-1, self.hidden_dimensions[5], 4, 4)
...
def main():
    ...
    model = HalfInputSmallFilterSizesSmallBottleneckVAE({
        "input_channels": 3,
        "latent_size": 32,
