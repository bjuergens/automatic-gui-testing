...
class HalfInputSmallFilterSizesSmallBottleneckMaxPoolVAE(BaseVAE):
    ...
    def __init__(self, model_parameters: dict):
        ...
        self.fc_log_var = nn.Linear(3 * 3 * self.hidden_dimensions[5], self.latent_size)

        self.fc_decoder = nn.Linear(self.latent_size, 3 * 3 * self.hidden_dimensions[5])

        transposed_conv_1 = nn.ConvTranspose2d(in_channels=self.hidden_dimensions[5], out_channels=self.hidden_dimensions[4], kernel_size=3, stride=2, padding=0, output_padding=0)
    ...
    def decode(self, z: torch.Tensor) -> torch.Tensor:
        x = self.fc_decoder(z)
        x = x.view(-1, self.hidden_dimensions[5], 3, 3)
...
def main():
    ...
    model = HalfInputSmallFilterSizesSmallBottleneckMaxPoolVAE({
        "input_channels": 3,
        "latent_size": 32,
